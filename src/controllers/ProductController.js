...
const AdjustPrice = require("../models/schema/AdjustPrice");
...
const addProductStageOne = async (req, res, next) => {
  ...
  try {
    if (action === "insert") {
      ...
      const product = new ProductStandardSchema({
        id: seq,
        name: nameProduct,
        breadcrumbs,
        url_path,
      ...
      const productUpdate = await ProductStandardSchema.findOneAndUpdate(
        { _id: productId },
        { name: nameProduct },
        { new: true }
      );
      ...
      const productUpdateBreadCrumbs =
        await ProductStandardSchema.findOneAndUpdate(
          { _id: productId },
          { breadcrumbs, url_path },
          { new: true }
        );
...
const addProductStageTow = async (req, res, next) => {
  ...
  const updated = await ProductStandardSchema.findOneAndUpdate(
    { _id: productId },
    { description, specs }
  );
  res.json(updated);
...
const addProductStageThree = async (req, res, next) => {
  ...
  try {
    const productUpdated = await ProductStandardSchema.findOneAndUpdate(
      { _id: productId },
      { price: singlePrice, discountRange: rangerPrice },
      { new: true }
    );
    const insertedInventory = await InventorySchema.create({
      productId: productUpdated.id,
      quantity: singleStock,
    });
...
const add_model = async (req, res, next) => {
  ...
  const updated = await ProductStandardSchema.findOneAndUpdate(
    { _id: productId },
    { models, totalStock, price: avgPrice },
    { new: true }
  );
  ...
  Promise.all(arrPromise).then(async (result) => {
    try {
      const productModelInsert = await ProductStandardSchema.insertMany(
        arrInsertProduct
      );
      const inventoryInserted = await InventorySchema.insertMany(
        arrInsertInventory
      );
      const updatedChild = await ProductStandardSchema.findOneAndUpdate(
        { _id: productId },
        { child: newproductId }
      );
      return res.json(updated._id);
...
const uploadImgProduct = async (req, res) => {
  ...
  if (req.method === "POST") {
    ...
    for (let i = 0; i < updated.child.length; i++) {
      ...
      let imgOfChild =
        updated.img[Math.floor(Math.random() * items.updated.img)];
      let updatedChild = await ProductStandardSchema.findOneAndUpdate(
        { id: childId },
        { img: [imgOfChild] }
      );
    }
...
const adjustPrice = async (req, res, next) => {
  const { idProduct, adjustedPrice } = req.body;
  try {
    ...
    //check cureent day and month
    const checkAdjust = await AdjustPrice.aggregate(
      [
        {$project: {idProduct:1,createdAt:1, 'day': {$dayOfMonth:'$createdAt' }, 'month': {$month: '$createdAt'}}},
        {$match: { idProduct:idProduct * 1, month: currentMonth, day: dayofMonth}}
      ]
    )
    if(checkAdjust.length <= 0) {  
          const adjustPriceProduct = await ProductStandardSchema.findOneAndUpdate(
            { id: idProduct, price: { $ne: adjustedPrice } },
            { price: adjustedPrice }
          );
          if (adjustPriceProduct) {
            const historyAdjust = await AdjustPrice.create({idProduct,originPrice: adjustPriceProduct.price, adjustedPrice})
            if (historyAdjust) {
              return res.json({ code: 200, message: historyAdjust });
            }
          }
    } else {
      return res.json({ code: 304, message: "Chỉ có thể thay đổi 1 lần/ngày"});
    }
    return res.json({ code: 500, message: "Canot update" });
  } catch (error) {
    console.log(error);
...
const changePrice = async (req, res, next) => {
  try {
    const { changePrice, priceAdjust } = req.body;
    for (let i = 0; i < changePrice.length; i++) {
      const adjustPriceProduct = await ProductStandardSchema.findOneAndUpdate(
        { id: changePrice[i], price: { $ne: priceAdjust } },
        { price: priceAdjust }
      );
      if (adjustPriceProduct) {
        const historyAdjust = await AdjustPrice.findOneAndUpdate(
          { idProduct: changePrice[i] },
          { originPrice: adjustPriceProduct.price, adjustedPrice: priceAdjust },
          { upsert: true, new: true }
        );
      }
    }
    return res.redirect("/admin/san-pham-kho");
  } catch (error) {
    next(error);
  }
};
const historyPrice = async(req, res, next) => {
  const {idProduct} = req.body;
  try {
    const dataHistory = await AdjustPrice.find({idProduct});
    res.json({code:200, message:dataHistory});
  } catch (error) {
    next(error);
  }
